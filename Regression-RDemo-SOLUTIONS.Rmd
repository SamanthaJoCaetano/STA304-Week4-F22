...
Let's start by playing around with some SRS ratio estimation. Use the `svyglm()` function in the `survey` library to run a ratio estimation of mass given height. Assume it was a Simple Random Sample and use the finite population correction with $N=200$ and assuming 25% of the population are "feminine" and 75% are masculine.
...
starwars <- starwars %>% 
  mutate(pop_strata_weight = case_when(gender=="feminine" ~ 25,
                                   gender=="masculine" ~ 75))

starwars <- starwars %>% 
  mutate(samp_strata_weight = case_when(gender=="feminine" ~ 25/17,
                                   gender=="masculine" ~ 75/66))

starwars <- starwars %>% filter(!is.na(gender))
...
### Recall SRS code (if you want to compare results)
